...
export default function AdminAnalyticsPage() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      // 1. Fetch Visitor Stats from new sessions and page_views tables
      ...
      if (visits && activeSessions) {
        const now = new Date()
        const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime()
        const weekStart = todayStart - 7 * 24 * 60 * 60 * 1000
        const fiveMinsAgo = now.getTime() - 5 * 60 * 1000

        const live = activeSessions.filter(s => new Date(s.last_seen).getTime() >= fiveMinsAgo).length
        ...
        setStats({ live, today, week, total: activeSessions.length })
      ...
      // 2. Fetch Top Content Engagement from material_views
      const { data: views } = await supabase
        .from("material_views")
        .select("material:content_materials(type, title)")
      
      if (views) {
        // Simplify for display (grouped by material type)
        const typeMap = new Map()
        views.forEach(v => {
          const mType = v.material?.type || "Unknown"
          typeMap.set(mType, (typeMap.get(mType) || 0) + 1)
        })
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: "Today's Visits", value: stats.today, icon: Clock, color: "bg-primary", desc: "Past 24 hours" },
              ...
              <Card key={stat.label} className="overflow-hidden border-none bg-muted/20">
                <CardContent className="p-6 relative">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Badge variant="outline" className="text-[10px] uppercase border-slate-200 dark:border-white/10">{stat.desc}</Badge>
                  </div>
