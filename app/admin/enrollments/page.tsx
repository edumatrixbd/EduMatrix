    ...
    <div className="space-y-8">
      {loading ? (
        ...
        <>
          ...
          {/* Filters */}
          <Card className="border-slate-200 dark:border-white/5 bg-white/50 dark:bg-card/50 backdrop-blur-sm">
            <CardContent className="p-4 flex flex-col md:flex-row items-center gap-4">
              <div className="relative flex-1 w-full">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input 
                  placeholder="Search student or course..." 
                  className="pl-9 bg-muted/30 border-slate-200 dark:border-white/10"
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                />
              </div>
              <Select value={selectedCourse} onValueChange={setSelectedCourse}>
                <SelectTrigger className="w-full md:w-[240px] bg-muted/30 border-slate-200 dark:border-white/10">
              ...
              <Button variant="outline" className="border-slate-200 dark:border-white/10 w-full md:w-auto">
                  ...
                  <TableBody>
                    {filteredEnrollments.length === 0 ? (
                      ...
                      filteredEnrollments.map((e) => (
                        <TableRow key={e.id} className="border-slate-200 dark:border-white/5 hover:bg-slate-100 dark:hover:bg-white/[0.01] transition-colors">
                          <TableCell>
                          ...
                          <TableCell>
                            <div className="flex flex-col gap-1.5">
                              ...
                              <Progress value={e.progress} className="h-1.5 bg-slate-50 dark:bg-white/5" />
                            </div>
                          </TableCell>
